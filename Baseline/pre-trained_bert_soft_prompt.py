...
SAVE_DIR = '../venn_models_softprompt_bert'
NUM_FOLDS = 10
...
class BertForMultiLabelSoftPrompt(PreTrainedModel):
    def __init__(self, config, prompt_text="Analyze defense mechanism patches", num_labels=6):
        ...
        self.prompt_tokens = self.tokenizer.encode(
            self.prompt_text, 
            add_special_tokens=False,
            return_tensors="pt"
        )
        self.prompt_length = self.prompt_tokens.size(1)
    ...
    def get_input_embeddings(self):
        if self.config.model_type == 't5':
            return self.bert.shared
        elif hasattr(self.bert, 'embeddings'):
            if hasattr(self.bert.embeddings, 'word_embeddings'):
                return self.bert.embeddings.word_embeddings
            else:
                return self.bert.embeddings
        elif hasattr(self.bert, 'wte'):
             return self.bert.wte
        elif hasattr(self.bert, 'get_input_embeddings'):
            return self.bert.get_input_embeddings()
        else:
            raise AttributeError("Could not find input embeddings layer")

    def forward(self, input_ids, attention_mask=None, labels=None):
        batch_size = input_ids.size(0)
        device = input_ids.device
        
        self.prompt_tokens = self.prompt_tokens.to(device)
        input_embeddings_layer = self.get_input_embeddings()
        prompt_embeds = input_embeddings_layer(self.prompt_tokens).expand(batch_size, -1, -1)
        
        inputs_embeds = input_embeddings_layer(input_ids)
...
def evaluate_model(model, data_loader, criterion, device, thresholds=None):
    ...
    results = {}
    for i, label_name in enumerate(LABEL_NAMES):
        precision, recall, f1, _ = precision_recall_fscore_support(all_labels[:, i], all_preds[:, i], average='binary', zero_division=0)
        accuracy = accuracy_score(all_labels[:, i], all_preds[:, i])
        results[label_name] = {'precision': precision, 'recall': recall, 'f1': f1, 'accuracy': accuracy, 'threshold': thresholds[i]}
    overall_precision, overall_recall, overall_f1, _ = precision_recall_fscore_support(all_labels.flatten(), all_preds.flatten(), average='binary', zero_division=0)
    overall_accuracy = accuracy_score(all_labels.flatten(), all_preds.flatten())
    results['overall'] = {'precision': overall_precision, 'recall': overall_recall, 'f1': overall_f1, 'accuracy': overall_accuracy, 'loss': avg_loss, 'thresholds': thresholds}
...
def train_and_evaluate_10fold():
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(full_dataset)):
        ...
        fold_train_data, fold_val_data = train_test_split(fold_train_val_data, test_size=1/9, random_state=42)
        ...
        model = BertForMultiLabelSoftPrompt(config, prompt_text=PROMPT_TEXT, num_labels=len(LABEL_NAMES))
        model.to(DEVICE)
        ...
        for epoch in range(EPOCHS):
            ...
            val_results, _, _, val_thresholds = evaluate_model(model, val_loader, criterion, DEVICE)
        ...
        test_results, test_preds, _, _ = evaluate_model(model, test_loader, criterion, DEVICE, best_val_thresholds)
        ...
        for i, item in enumerate(fold_test_data):
            ...
            fold_results.append({'fold': fold + 1, 'cve': item['cve'], 'commit1': item['commit1'], 'commit2': item['commit2'], 'true_labels': item['labels'], 'pred_labels': binary_preds, 'probs': probs, 'thresholds': best_val_thresholds})

    print("\n===== Dictionary-based Evaluation =====")
    
    cve_data = defaultdict(lambda: {'true': defaultdict(lambda: defaultdict(list)), 'pred': defaultdict(lambda: defaultdict(list)), 'true_separate': set(), 'pred_separate': set()})
    ...
    for col, ans_col in zip(cols, ans_cols):
        if col in df_eval.columns:
            try:
                ...
                precision, recall, f1, tp, pred, gold, error_cves = evaluate_dict_column(true_series, pred_series, col, df_eval)
                print(f"{col}: P={precision:.4f}, R={recall:.4f}, F1={f1:.4f}")
                results[col] = {'precision': precision, 'recall': recall, 'f1': f1, 'accuracy': 0}
            except Exception as e:
                print(f"Error evaluating {col}: {e}")

    if 'separate' in df_eval.columns and 'ans_separate' in df_eval.columns:
        try:
            precision, recall, f1, tp, pred, gold = evaluate_separate_column(df_eval['separate'], df_eval['ans_separate'])
            print(f"separate: P={precision:.4f}, R={recall:.4f}, F1={f1:.4f}")
            results['separate'] = {'precision': precision, 'recall': recall, 'f1': f1, 'accuracy': 0}
        except Exception as e:
        ...
        for label in LABEL_NAMES:
            ...
            if label == 'separate':
                ...
                for item in t_list:
                    ...
                    target_list.append({'uid': uid, 'cve': cve, 'label': label, 'item': item, f'{MODEL_NAME}': is_correct})
            else:
                ...
                for c1, c2_list in t_dict.items():
                    ...
                    target_list.append({'uid': uid, 'cve': cve, 'label': label, 'item': f"{c1}->[{t_vals_str}]", f'{MODEL_NAME}': is_correct})
...
def predict_relationship(commit1_id, commit2_id, model_path=None, thresholds=None):
    ...
    commit1_msg = clean_text(commit1_info.get('message', commit1_info.get('msg_text', '')))
    commit1_diff = clean_text(commit1_info.get('diff_code', ''))
    commit1_time = commit1_info.get('commit_time', '')
    commit1_branch = commit1_info.get('branch', '')
    commit1_tags = commit1_info.get('tags', '')
    
    commit1_text = (f"COMMIT ID: {commit1_id}\nCOMMIT TIME: {commit1_time}\nBRANCH: {commit1_branch}\nVERSION: {commit1_tags}\nMESSAGE: {commit1_msg}\nDIFFCODE: {commit1_diff}")
    
    commit2_msg = clean_text(commit2_info.get('message', commit2_info.get('msg_text', '')))
    commit2_diff = clean_text(commit2_info.get('diff_code', ''))
    commit2_time = commit2_info.get('commit_time', '')
    commit2_branch = commit2_info.get('branch', '')
    commit2_tags = commit2_info.get('tags', '')
    
    commit2_text = (f"COMMIT ID: {commit2_id}\nCOMMIT TIME: {commit2_time}\nBRANCH: {commit2_branch}\nVERSION: {commit2_tags}\nMESSAGE: {commit2_msg}\nDIFFCODE: {commit2_diff}")

    combined_text = commit1_text + "\n\n[SEP]\n\n" + commit2_text
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)
    config = AutoConfig.from_pretrained(MODEL_NAME)
    model = BertForMultiLabelSoftPrompt(config, prompt_text=PROMPT_TEXT, num_labels=len(LABEL_NAMES))
    ...
    encoding = tokenizer(combined_text, truncation=True, max_length=MAX_LENGTH, padding='max_length', return_tensors='pt')
    with torch.no_grad():
    ...
    results = {}
    for i, label_name in enumerate(LABEL_NAMES):
        results[label_name] = {'probability': float(probs[i]), 'prediction': bool(preds[i]), 'threshold': float(thresholds[i])}
    return results
