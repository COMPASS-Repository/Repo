...
SAVE_DIR = '../venn_models_softprompt_codereviewer'
NUM_FOLDS = 10
...
def prepare_dataset():
    ...
    for _, row in commit_details_df.iterrows():
        ...
        try:
            commit_short = str(commit_id).strip()[:7]
            full_commit_text = (
                f"Commit ID: {commit_id}\n"
                f"Commit Time: {commit_time}\n"
                f"Branch: {branch}\n"
                f"Version: {tags}\n"
                f"Message: {clean_text(msg_text)}\n"
                f"Code Diff: {clean_text(diff_code)}"
            )
            commit_info[commit_short] = full_commit_text
        except Exception:
            pass
    ...
    for _, row in labels_df.iterrows():
        ...
        if text1 and text2:
            combined_text = CUSTOM_PROMPT + " [SEP] " + text1 + " [SEP] " + text2
...
class BertForMultiLabelSoftPrompt(PreTrainedModel):
    def __init__(self, config, soft_prompt_length=10, num_labels=6):
        ...
        self.soft_prompt = torch.nn.Embedding(soft_prompt_length, config.hidden_size)
        torch.nn.init.normal_(self.soft_prompt.weight, std=0.02)
    ...
    def get_input_embeddings(self):
        if self.config.model_type == 't5':
            ...
            raise AttributeError("Could not find input embeddings layer for this model architecture")

    def forward(self, input_ids, attention_mask=None, labels=None):
        batch_size = input_ids.size(0)
        device = input_ids.device
        
        soft_prompt_ids = torch.arange(self.soft_prompt_length, device=device).unsqueeze(0).expand(batch_size, -1)
        soft_prompt_embeds = self.soft_prompt(soft_prompt_ids)
        ...
        if attention_mask is not None:
            soft_prompt_mask = torch.ones((batch_size, self.soft_prompt_length), device=device)
...
def train_and_evaluate_10fold():
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(full_dataset)):
        ...
        model = BertForMultiLabelSoftPrompt(config, soft_prompt_length=SOFT_PROMPT_LENGTH, num_labels=len(LABEL_NAMES))
        model.to(DEVICE)
