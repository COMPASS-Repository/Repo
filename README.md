...
![image6](Src_png/Overview_fig.png)

- **Phase-1: Patch Group Extraction & Pre-Scanning**
  - The method takes a vulnerability entry from public platforms (e.g., `CVE`, `NVD`, `Snyk`) as input. These platforms typically record links to the patches that fix the vulnerability, allowing the required patch group to be obtained directly. This followed by determining some explicit patch relationships at an early stage via a pre-scanning step. Then, the vulnerability (including `the CVE ID` and `the description`) and its patch group (including `the commit ID`, `the branch/version information`, `the commit message`, and `the code change of each patch`) will be fed into an LLM under the intuition of chain-of-thought.
  - Source code: **[Phase1_code.py](COMPASS/Phase-1/Phase1_code.py)**.
    
- **Phase-2: Individual Patch Analysis**
  - We ask the LLM to double-check the patch group provided by the vulnerability platform for further determining whether each patch directly fixes the vulnerability. And we also ask the LLM to identify key components and files modified, and note any special markers, to facilitate the following analysis.
...
| Search CVE | A search box for CVE lookup in the dataset  | ![image2](Src_png/SearchCVE.png) | [Search CVE](https://patch-relation.com/#/SearchCVE) |
...
```bash
├── *Baseline/*
│   ├── pre-trained_bert.py                        # BERT-based baseline
│   ├── pre-trained_bert_soft_prompt.py            # BERT (with soft-prompt)-based baseline
│   ├── pre-trained_codet5.py                      # CodeT5-based baseline
│   ├── pre-trained_codet5_soft_prompt.py          # CodeT5 (with soft-prompt)-based baseline
│   ├── pre-trained_codereviewer.py                # CodeReviewer-based baseline
│   ├── pre-trained_codereviewer_soft_prompt.py    # CodeReviewer (with soft-prompt)-based baseline
│   ├── ds_only.py                                 # LLM-only baseline
│
├── *COMPASS/*
│   ├── Phase-1/*
...
│
├── *Prompt_Phase-3*                # The prompt used in Phase-3 (Patch Relationship Prediction)
│
